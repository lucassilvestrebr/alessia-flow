...
function Logos() {
  const companies = [
  { n: "Veross", f: "logo-veross.svg", h: 28 },
  { n: "Inbotix", f: "logo-inbotix.png", h: 28 },
  { n: "Mag Seguros", f: "mag-seguros.svg", h: 34 },
  { n: "SocialPlay Marketing", f: "logo-socialplay.png", h: 30 },
  { n: "Tibox", f: "logo-tibox.png", h: 32 },
  { n: "Plataforma Empresas", f: "logo-plataforma-empresas.png", h: 26 },
  { n: "Ademicon Penha", f: "logo-ademicon.svg", h: 28 },
  { n: "Mirai Dev.AI", f: "logo-mirai.png", h: 30 },
  { n: "Careon Brasil", f: "logo-careon.svg", h: 30 },
  { n: "ContaFácil", f: "logo-contafacil.png", h: 24 },
  { n: "Kash Academy", f: "logo-kash-academy.png", h: 30 },
  { n: "Abellie Produções & Eventos", f: "logo-abellie.png", h: 32 }];
  ...
  useEffect(() => {
    ...
    const reduce = false;
    let unit = track.scrollWidth / 3;
    let x = 0, drift = -48, vel = 0, dragging = false, lastX = 0, lastT = 0, raf = 0, prev = 0;

    const measure = () => {unit = track.scrollWidth / 3;};
    const wrap = (v) => {if (!unit) return 0;v = v % unit;return v > 0 ? v - unit : v;};
    ...
    const frame = (t) => {
      ...
      if (dragging) {
        ...
        if (Math.abs(vel) > 12) {vel *= 0.94;} else {vel = 0;}
        x += (vel + (reduce ? 0 : drift)) * dt;
        x = wrap(x);
    ...
    const down = (e) => {
      ...
      marq.classList.add("dragging");
      try {marq.setPointerCapture(e.pointerId);} catch (err) {}
    };
    const move = (e) => {
      if (!dragging) return;
      const now = performance.now(), dx = e.clientX - lastX, dt = Math.max(now - lastT, 1);
      x = wrap(x + dx);
      vel = dx / dt * 1000;
    ...
    const up = (e) => {
      if (!dragging) return;
      dragging = false;
      marq.classList.remove("dragging");
      try {marq.releasePointerCapture(e.pointerId);} catch (err) {}
      if (performance.now() - lastT > 90) vel = 0;
    };

    marq.addEventListener("pointerdown", down);
    marq.addEventListener("pointermove", move);
    marq.addEventListener("pointerup", up);
    marq.addEventListener("pointercancel", up);
    window.addEventListener("resize", measure);
    const imgs = marq.querySelectorAll("img");
    imgs.forEach((im) => im.addEventListener("load", measure));
    marq.addEventListener("dragstart", (e) => e.preventDefault());

    return () => {
      cancelAnimationFrame(raf);
      marq.removeEventListener("pointerdown", down);
      marq.removeEventListener("pointermove", move);
      marq.removeEventListener("pointerup", up);
      marq.removeEventListener("pointercancel", up);
  ...
  return (
    <section className="logos">
      ...
      <div className="logos-marquee" ref={marqRef}>
        <div className="logos-track" ref={trackRef}>
          {[0, 1, 2].map((dup) =>
          <div className="logos-row" key={dup} aria-hidden={dup !== 0}>
            {companies.map((c, i) =>
            <div key={i} className="logo-item"><img src={"assets/img/clientes/" + c.f} alt={c.n} draggable="false" style={{ height: c.h }} /></div>
            )}
          </div>
          )}
        </div>
  ...
  return (
    <section className="section" id="pricing">
      ...
      <p style={{ textAlign: "center", marginTop: 32, fontSize: 13, color: "var(--ink-3)" }}>
        Preços em BRL. Operado pela Veross Outbound Sales — CNPJ 47.876.266/0001-66. Cancelamento self-service. <a href="legal/politica-de-cancelamento.html" style={{ color: "var(--ink-3)", textDecoration: "underline" }}>Política de cancelamento</a>.
      </p>
    </section>);
...
function FAQ() {
  ...
  const items = [
  { q: "O LinkedIn não bane contas que usam automação?", a: "O LinkedIn bane uso descuidado de automação — extensões de navegador, IPs compartilhados e ritmos sobre-humanos. O AlessIA Flow opera 100% em nuvem, com IP brasileiro fixo e exclusivo por perfil, dentro dos limites diários seguros. Não tivemos um único banimento em mais de 48 meses." },
  ...
  { q: "Funciona com o Sales Navigator?", a: "Sim. A AlessIA Flow funciona com contas básicas do LinkedIn, mas recomendamos o LinkedIn Premium — o mínimo indicado para melhor performance é o plano Business. Importe buscas direto do Sales Navigator, conecte por URL ou via integração nativa. Também aceitamos CSV, conexão a CRM (HubSpot, Salesforce, Pipedrive, RD) e webhooks." },
  { q: "Como é o suporte?", a: "100% em português. Suporte via grupo exclusivo no WhatsApp com humanos e IA (beta), além de tutoriais e conteúdos no YouTube. Chat ao vivo (em breve). Planos Pro têm acesso à comunidade fechada e masterclasses semanais. Plano Agency inclui gerente de conta dedicado." },
    ...
    <footer className="footer">
      <div className="footer-inner">
        ...
        <div className="footer-col">
          ...
          <a href="legal/politica-de-privacidade.html">Privacidade</a>
          <a href="legal/politica-de-cancelamento.html">Cancelamento</a>
          <a href="#">Segurança</a>
        </div>
      </div>
      <div className="footer-bottom">
        <span>© 2026 Veross Outbound Sales · CNPJ 47.876.266/0001-66 · alessiaflow@veross.com.br · +55 12 98251-9830</span>
        <span><a href="legal/politica-de-privacidade.html" style={{ color: "inherit" }}>Privacidade</a> · <a href="legal/termos-de-uso.html" style={{ color: "inherit" }}>Termos</a> · <a href="legal/politica-de-cancelamento.html" style={{ color: "inherit" }}>Cancelamento</a></span>
      </div>
    </footer>);
